        ...
        <div class="product-card">
            ...
            <div>
                ...
                <div id="controls-${taco.nombre.replace(/\s/g, "")}" class="btn-wrap">
                    <button class="btn-primary flex-btn" onclick="agregarAlCarrito(decodeURIComponent('${itemInfoStr}'))">Agregar al pedido</button>
                </div>
            </div>
        </div>`;
    });
...
// ==========================================
window.agregarAlCarrito = function (itemJsonData) {
    let item;
    try {
        // En caso de que recibamos un JSON str normal
        item = JSON.parse(itemJsonData);
    } catch {
        // Fallback porsia es el uri encoded viejo
        item = JSON.parse(decodeURIComponent(itemJsonData));
    }
    ...
    [...state.menu.tacos, ...state.menu.bebidas].forEach(item => {
        ...
        if (cartItem && cartItem.cant > 0) {
            ...
            <div class="qty-controls">
                ...
                <button class="qty-btn" onclick="agregarAlCarrito('${cartItem.fullItem.replace(/'/g, "\\'")}')">＋</button>
            </div>
            `;
        } else {
            // Mostrar boton normal
            const itemJson = JSON.stringify(item);
            // Escapar comillas que rompan el HTML
            const safeJson = itemJson.replace(/'/g, "&#39;").replace(/"/g, "&quot;");
            cnt.innerHTML = `<button class="btn-primary flex-btn" style="padding:10px;" onclick="agregarAlCarrito('${safeJson}')">Agregar al pedido</button>`;
        }
    });
